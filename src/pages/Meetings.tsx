...
export default function Meetings() {
  ...
  const normalizeRoomId = (code: string): string => {
  ...
  const handleJoinPersistentRoom = async (room: any) => {
    setRoomsModalVisible(false);
    setMeetingLoading(true);
    try {
      const response = await api.meetings.validateMeeting(room.id, undefined);
      setActiveMeetingRoom({
        id: response._id || response.roomId || room.id,
        title: response.title || room.title,
        roomId: response.joinCode || room.id,
        password: '',
        time: 'Persistent Room',
        duration: 'Unlimited',
        attendees: response.participantIds?.length || room.activeMembers + 1,
        color: '#10b981'
      });
    } catch (err: any) {
      console.warn("Failed to join persistent room, using local offline backup:", err);
      setActiveMeetingRoom({
        id: room.id,
        title: room.title,
        roomId: room.id,
  ...
  const handleJoinUpcomingMeeting = async (meeting: any) => {
    setMeetingLoading(true);
    try {
      const roomCode = String(meeting.id || meeting.roomId || '').trim();
      const response = await api.meetings.validateMeeting(roomCode, undefined);
      setActiveMeetingRoom({
        id: response._id || response.roomId || meeting.id,
        title: response.title || meeting.title,
        roomId: response.joinCode || meeting.roomId || meeting.id,
        password: '',
        time: 'Live Now',
        duration: `${response.durationMinutes || 60}m`,
        attendees: response.participantIds?.length || meeting.attendees,
        color: meeting.color || '#10b981'
      });
    } catch (err: any) {
      console.warn("Failed to register and join upcoming meeting on server:", err);
  ...
  const handleJoinMeeting = async () => {
    ...
    setMeetingLoading(true);
    try {
      const response = await api.meetings.validateMeeting(normalized, joinPasscode || undefined);
      setJoinModalVisible(false);
      setActiveMeetingRoom({
        id: response._id || response.roomId || normalized,
        title: response.title || 'Joined Session',
        roomId: response.joinCode || normalized,
      ...
      Alert.alert("Secure Connection established", `Joined Room ${normalized} successfully via offline backup!`);
    } finally {
      setMeetingLoading(false);
    }
  };
            ...
            <View style={styles.roomsList}>
              {persistentRooms.map((room) => (
                <TouchableOpacity 
                  ...
                  onPress={() => handleJoinPersistentRoom(room)}
                >
